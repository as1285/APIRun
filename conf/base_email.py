...
def send_mail(**kwargs):
    ...
    msg.attach(MIMEText(kwargs["attach"], 'html', 'utf-8'))

    if kwargs.get("report", "0") != "0":
        part = MIMEApplication(open(kwargs["report"], 'rb').read())
        part.add_header('Content-Disposition', 'html', filename=('utf-8', '', kwargs["report_name"]))
        msg.attach(part)
...
if __name__ == '__main__':
    to_addr = ["dong1285@126.com"]
    mail_host = "smtp.qq.com"
    mail_user = '498771018@qq.com'
    mail_pass = 'ikcwnwolsdzjbicf'
    port = "465"
    header_msg = "接口测试"
    attach = "接口测试"
    report = PATH("../report/result.html")
    send_mail(to_addr = to_addr, mail_host = mail_host, mail_user=mail_user, port=port, mail_pass=mail_pass, header_msg=header_msg, report=report, attach=attach, report_name="接口报告")
